...
async function Read({ params }: { params: { slug: string }; }) {
  ...
  const getPost:any = await getSinglePost(params.slug)
  // console.log(getPost)
            ...
            <header className="mb-4 lg:mb-6 not-format">
              ...
              <address className="flex items-center mb-6 not-italic">
                ...
                <div className="inline-flex items-center mr-3 text-sm text-gray-900 dark:text-white">
                  ...
                  {
                    getPost?.primary_author ? <Link href={`/author/${getPost?.primary_author.slug}`} rel="author" className="text-xl font-bold text-gray-800 dark:text-white">{getPost?.primary_author.name}</Link> : " "
                  }
