...
import { Key, ReactElement, JSXElementConstructor, ReactFragment, PromiseLikeOfReactNode } from "react";
import { UrlObject } from "url";

function Header({ setting }: { setting: any }) {
  // console.log(setting);
        ...
        <div className="flex md:order-2">
          ...
          <ul className="flex flex-wrap p-4 md:space-x-8 md:mt-0 md:text-sm md:font-medium">

            {
              ...
              setting.navigation !== undefined ? setting?.navigation.map((item: any) => <li key={item?.label} className="block py-2 pl-3 pr-4 text-gray-700 rounded hover:text-blue-700 dark:hover:text-blue-700 md:p-0 dark:text-white"
                aria-current="page">
                <Link href={item?.url}>
                  {item?.label}
                </Link>
